...
Private Sub Version(ByVal Sender As User)
    Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, AppName & "-" & AppVersion & "[" & AppCompileInfo & "] - " & basMain.Service(5).Nick & "[" & sAdminServ.ModVersion & "]")
End Sub
    ...
    Select Case UCase(Action)
        Case "ABUSETEAMADD"
            If Users.Exists(TargetNick) = False Then
                Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replies.UserDoesntExist)
                Exit Function
            End If
            ...
            End If
            Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replace(Replies.AdminServUserAddToAbuseTeam, "%n", TargetNick))
        Case "ABUSETEAMDEL"
            If Users.Exists(TargetNick) = False Then
                Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replies.UserDoesntExist)
                Exit Function
...
Private Function Access(ByVal Sender As User, TargetNick As String, NewAccess As String)
    ...
    If (InStr(1, NewAccess, AccFlagCoMaster) > 0 Or InStr(1, NewAccess, AccFlagMaster) > 0) And Not Sender.HasFlag(AccFlagMaster) Then
        ...
        Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replies.AdminServCantAddCoMaster)
        Exit Function
    End If
    ...
    If Successful = True Then
        Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replace(Replies.AdminServAccessModified, "%n", TargetNick))
    Else
        Call basFunctions.SendMessage(basMain.Service(5).Nick, Sender.Nick, Replies.UserDoesntExist)
    End If
End Function
